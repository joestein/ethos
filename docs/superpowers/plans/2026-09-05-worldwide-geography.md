...
- **A late-arriving corpus also needs its town-hub legacy paths, and nothing automated produces them.** Task 9 mapped 422 old `/destinations/:slug` town hubs onto node `legacy_paths` using a scratch derivation, not a gate — so a guide that lands after Task 9 gets no legacy path, and its pre-refactor URL 404s. When merging a corpus from another worktree, re-derive the mapping the same way Task 9 did: for every guide, slug the first comma-segment of `destination` with `Guide.derive_destination_slug/1`, and add it to that guide's node unless it is already a node path, already a legacy path, or collides with another node (Task 9 found exactly one collision, `madison`, and left it a deliberate 404). The roster-uniqueness test added in Task 9 will catch a duplicate or a node-path shadow, but it cannot tell you about a mapping you never wrote.
