defmodule Mix.Tasks.Ethos.OptimizeQueensPhotos do
  @shortdoc "Optimizes Queens photos from images/queens/ into priv/photos/ny/queens/, driven by seed JSONs"
  @moduledoc "Scans priv/seed_data/queens/*.json for photo srcs and optimizes images/queens/{label}.* accordingly."
  use Mix.Task

  @source_exts ~w(jpg jpeg png tif tiff JPG JPEG PNG)
  @manifest_path "priv/seed_data/queens_photo_manifest.json"

  @impl true
  def run(_args) do
    files = Path.wildcard("priv/seed_data/queens/*.json")

    if files == [] do
      Mix.raise(
        "no seed files matched priv/seed_data/queens/*.json — run this from the project root"
      )
    end
  ...
  defp optimize_src(src) do
    case String.split(src, "/") do
      ["", "photos", "ny", "queens", neighborhood, file] ->
        label = Path.rootname(file)
        out_dir = Path.join(["priv", "photos", "ny", "queens", neighborhood])
      ...
      _ ->
        Mix.raise(
          "unexpected photo src path #{src} (want /photos/ny/queens/{neighborhood}/{label}.jpg)"
        )
    end
  end
  ...
  # weaker than the rest. The manifest path is the only Queens-specific part.
  defp find_source!(label) do
    path =
      Enum.find_value(@source_exts, fn ext ->
        path = Path.join(["images", "queens", "#{label}.#{ext}"])
        if File.exists?(path), do: path
      end) || Mix.raise("no source image for #{label} under images/queens/")
